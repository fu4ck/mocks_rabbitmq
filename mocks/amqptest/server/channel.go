...
type (
	Channel struct {
		*VHost
		...
		publishListeners   []chan mocks.Confirmation
	...
	consumer struct {
		tag        string
		deliveries chan mocks.Delivery
		done       chan bool
	}
)
...
func (ch *Channel) NotifyPublish(confirm chan mocks.Confirmation) chan mocks.Confirmation {
	aux := make(chan mocks.Confirmation, 2<<8)
...
func (ch *Channel) Consume(queue, consumerName string, _ mocks.Option) (<-chan mocks.Delivery, error) {
	var (
		c consumer
	)
...
func (ch *Channel) addUnacked(d mocks.Delivery, q *Queue) {
	ch.muUnacked.Lock()
...
func (ch *Channel) Close() error {
	...
	ch.publishListeners = []chan mocks.Confirmation{}
