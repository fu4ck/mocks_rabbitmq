...
func (conn *Conn) NotifyClose(c chan mocks.Error) chan mocks.Error {
	conn.errSpread.Add(c)
	return c
}

// AutoRedial mock the reconnection faking a delay of 1 second
func (conn *Conn) AutoRedial(outChan chan mocks.Error, done chan bool) {
	if !conn.hasAutoRedial {
		conn.mu.Lock()
...
func (conn *Conn) Channel() (mocks.Channel, error) {
