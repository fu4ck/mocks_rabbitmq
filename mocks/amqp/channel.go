...
func (ch *Channel) Publish(exc, route string, msg []byte, opt mocks.Option) error {
...
func (ch *Channel) NotifyPublish(confirm chan mocks.Confirmation) chan mocks.Confirmation {
...
func (ch *Channel) Consume(queue, consumer string, opt mocks.Option) (<-chan mocks.Delivery, error) {
	var (
	...
	deliveries := make(chan mocks.Delivery)
...
func (ch *Channel) ExchangeDeclare(name, kind string, opt mocks.Option) error {
	return ch.exchangeDeclare(name, kind, false, opt)
}

func (ch *Channel) ExchangeDeclarePassive(name, kind string, opt mocks.Option) error {
	return ch.exchangeDeclare(name, kind, true, opt)
}

func (ch *Channel) exchangeDeclare(name, kind string, passive bool, opt mocks.Option) error {
	var (
...
func (ch *Channel) QueueInspect(name string) (mocks.Queue, error) {
	q, err := ch.Channel.QueueInspect(name)
	return &Queue{&q}, err
}

func (ch *Channel) QueueUnbind(name, route, exchange string, _ mocks.Option) error {
	return ch.Channel.QueueUnbind(name, route, exchange, nil)
}

// QueueBind binds the route key to queue
func (ch *Channel) QueueBind(name, key, exchange string, opt mocks.Option) error {
	var (
		noWait bool
		args   amqp.Table
	)
...
func (ch *Channel) QueueDeclare(name string, opt mocks.Option) (mocks.Queue, error) {
	return ch.queueDeclare(name, false, opt)
}

// QueueDeclarePassive declares an existing AMQP queue
func (ch *Channel) QueueDeclarePassive(name string, opt mocks.Option) (mocks.Queue, error) {
	return ch.queueDeclare(name, true, opt)
}

func (ch *Channel) queueDeclare(name string, passive bool, opt mocks.Option) (mocks.Queue, error) {
	var (
...
func (ch *Channel) QueueDelete(name string, opt mocks.Option) (int, error) {
	var (
...
func (ch *Channel) NotifyClose(c chan mocks.Error) chan mocks.Error {
